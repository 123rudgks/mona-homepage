...
const Footer = ({ darkMode, lang = 'ko', admin }: Props) => {
  ...
  const { MENU } = useMenu({ lang: lang, admin });
  return (
    <div
      className={cn(
...
export const NavMenu = ({ darkMode, title, headerMenu }: FooterNavMenu) => {
  return (
    <div className="flex  flex-1 min-h-[390px]">
      ...
      <div className="flex flex-col gap-6  pl-[30px]">
        ...
        {headerMenu?.menus.map((menu) => {
          if (menu.label) {
            return (
              <div className="flex flex-col gap-7" key={menu.label}>
                <div
                  className={cn(
                    'gap-2 flex flex-col',
            ...
            return (
              <div className="flex flex-col gap-7" key={menu.text[0]}>
                <div
                  className={cn(
                    'gap-2 flex flex-col',
                    darkMode ? 'text-whiteAlpha-70' : 'text-blackAlpha-50',
                  ...
                  <div
                    key={menu.text[0]}
                    className={cn('flex flex-col gap-2 typo-TitleBold')}>
                    {menu.text.map((text, idx) => (
                      <Link
                        key={text}
                        href={menu.path[idx]}
